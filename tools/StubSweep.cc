...
void StubSweep::Initialize()
{
    ...
    fSweepCanvas = new TCanvas (cName, "Stub Sweep", 10, 0, 350, 350 );
    ...
    for (auto cBoard : fBoardVector)
    {
        for (auto cFe : cBoard->fModuleVector)
        {
            ...
            for (auto cCbc : cFe->fCbcVector)
            {
                ...
                //setCorrelationWinodwOffsets( cCbc , -1.0 , 0.0 , 0.0 , 0.0 );
                ...
                for ( uint8_t  cTestGroup = 0 ; cTestGroup < 8 ; cTestGroup++)
                {
                    ...
                    for ( unsigned int i = 0 ; i < cNumberOfPairs ; i++ )
                    {
                        ...
                        do
                        {
                            ...
                            cNhits = cEvents[0]->GetNHits ( cFeId, cCbcId );
                            cStubPosition = getStubPosition ( cEvents, cFeId, cCbcId, pNEvents);
                            cStubBend = cEvents[0]->StubVector (cFeId, cCbcId ) [0].getBend();
                        ...
                        //fillStubBendHist ( cCbc,  cChannelPair , cStubBend );
