    ...
    for (auto cBoard : fBoardVector)
    {
        for (auto cFe : cBoard->fModuleVector)
        {
            ...
            for (auto cCbc : cFe->fCbcVector)
            {
                ...
                cStubBendHist->SetMarkerStyle(4); cStubBendHist->SetMarkerSize(0.5); 
                cStubBendHist->SetLineColor(kBlue); cStubBendHist->SetMarkerColor(kBlue);
        ...
        for (auto cFe : cBoard->fModuleVector)
        {
            ...
            for (auto cCbc : cFe->fCbcVector)
            {
                ...
                for( uint8_t  cTestGroup = 0 ; cTestGroup < 8 ; cTestGroup++)
                {
                    ...
                    std::vector<uint8_t> cChannelVector;cChannelVector.clear();
                    ...
                    {
                        std::vector<uint8_t> cChannelPair; cChannelPair.clear();
